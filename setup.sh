...
clone_dotfiles() {
  ...
  local dotfiles_path="$(ghq root)/github.com/$GITHUB_USERNAME/examples.dotfiles"
  ...
  if [ -d "$dotfiles_path" ]; then
    warn "examples.dotfiles already exists：$dotfiles_path"
  ...
  if ghq get git@github.com:$GITHUB_USERNAME/examples.dotfiles.git 2>/dev/null; then
    ...
    ghq get https://github.com/$GITHUB_USERNAME/examples.dotfiles.git
